...
- All UI text goes through `t()` in `js/i18n.js`: English (default), Swedish, German, Spanish, Hindi and Tamil.
- Online lookups: OpenStreetMap Nominatim (addresses, destination), OSRM (routes) and Overpass (chargers, restaurants) only. Only positions/destination text are sent.
